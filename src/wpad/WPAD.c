...
BOOL IsControllerDataChanged(WPADControlBlock* p_wpd, void* p_curr, void* p_prev)
{
	...
	if (WPADiIsCoreFormat(fmt)) {
		...
		if (curr->err == WPAD_ERR_NONE && prev->err == WPAD_ERR_NONE) {
			...
			if (IsAnalogChanged((s32)(curr->fsAccX), (s32)(prev->fsAccX), 12)
			    | IsAnalogChanged((s32)(curr->fsAccY), (s32)(prev->fsAccY), 12)
...
{
	OSSwitchFiberEx((u32)alarm, (u32)context, 0, 0, (u32)WPADiManageHandler,
	                (u32)(__WPADiManageHandlerStack + sizeof(__WPADiManageHandlerStack)));
}
...
{
	return WUDSetSyncDeviceCallback(callback);
}
...
{
	return WUDSetClearDeviceCallback(callback);
}
...
s32 WPADiRetrieveChannel(u8 dev_handle)
{
	...
	for (i = 0; i < WPAD_MAX_CONTROLLERS; i++) {
		if (!memcmp(_scArray.devices[i + 10].addr, devAddr, 6)) {
	...
	for (i = 0; i < WPAD_MAX_CONTROLLERS; i++) {
		if (_chan_active_state[i] == 0) {
			_chan_active_state[i] = 1;
			memcpy(_scArray.devices[i + 10].addr, devAddr, 6);
			_scFlush = 1;
			return i;
		}
	}
...
{
	WPADControlBlock* p_wpd = _wpdcb[chan];
	memset(&_scArray.devices[chan + 10], 0, sizeof(SCBtDeviceInfoSingle));
	_scFlush = 1;
	WPADiDisconnect(chan, TRUE);
}
...
s32 WPADControlDpd(s32 chan, u32 command, WPADCallback callback)
{
	...
	static const u8 cfg1[5][9] = {
		0x02, 0x00, 0x00, 0x71, 0x01, 0x00, 0x64, 0x00, 0xfe, 0x02, 0x00, 0x00, 0x71, 0x01, 0x00,
		0x96, 0x00, 0xb4, 0x02, 0x00, 0x00, 0x71, 0x01, 0x00, 0xaa, 0x00, 0x64, 0x02, 0x00, 0x00,
		0x71, 0x01, 0x00, 0xc8, 0x00, 0x36, 0x07, 0x00, 0x00, 0x71, 0x01, 0x00, 0x72, 0x00, 0x20,
	};
...
void __WPADShutdown()
{
	...
	for (i = 10; i < 14; i++) {
		memset(&_scArray.devices[i], 0, sizeof(SCBtDeviceInfoSingle));
	}
