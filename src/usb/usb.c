...
typedef enum { USB_NCLEAN_CLOSEDEVICE = 0, USB_NCLEAN_BULKMSG = 3, USB_NCLEAN_CTRLMSG = 7, USB_NCLEAN_MAX = 8 } USBNClean;

typedef enum { USB_IOCTLV_CTRLMSG, USB_IOCTLV_BLKMSG, USB_IOCTLV_INTRMSG } USBIoctl;
...
typedef struct USBCommandBlock {
	...
	u32 nclean;                  // at 0x2C
	union {
		char path[ROUNDUP(FS_MAX_PATH)];
        char des[ROUNDUP(sizeof(USB_DevDescr))];
		USBMsg msg;
	} ATTRIBUTE_ALIGN(32); // at 0x40
} USBCommandBlock;
...
static s32 _intrBlkCtrlIsoCb(s32 result, void* arg)
{
	...
	} else if (block->icallback != NULL) {
        USB_LOG("calling iso callback\n");
        block->icallback(result, block->xfer, block->callbackArg);
    }
...
DECOMP_FORCEACTIVE(usb_c, "openDevice: Not enough memory\n", "getDeviceList: Not enough memory\n");

static IPCResult __IntrBlkMsgInt(s32 fd, u32 endpoint, u32 length, void* buffer, u8 ioctl, USBCallback callback, void* callbackArg,
                                 u8 async)
{
	IPCResult result;
	IPCIOVector* vectors;
...
IPCResult IUSB_ReadIntrMsgAsync(s32 fd, u32 endpoint, u32 length, void* buffer, USBCallback callback, void* callbackArg)
{
...
IPCResult IUSB_WriteBlkMsgAsync(s32 fd, u32 endpoint, u32 length, void* buffer, USBCallback callback, void* callbackArg)
{
...
static IPCResult __CtrlMsgInt(s32 fd, u8 requestType, u8 request, u16 value, u16 index, u16 length, void* buffer, USBCallback callback,
...
IPCResult IUSB_WriteCtrlMsgAsync(s32 fd, u8 requestType, u8 request, u16 value, u16 index, u16 length, void* buffer, USBCallback callback,
                                 void* callbackArg)
{
	DCFlushRange(buffer, length);
...
DECOMP_FORCEACTIVE(usb_c, "GetStrCb returned: %d\n", "GetStrCb: buf = 0x%x buflen = %u\n",
