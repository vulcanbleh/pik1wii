...
public:
	...
	u32 getRelease() const { return release; }
	u32 getTrigger() const { return trig; }

	bool down(u32 buttons) const { return hold & buttons; }
	bool up(u32 buttons) const { return (hold & buttons) != buttons; }

	bool downAll(u32 buttons) const { return (buttons & hold) == buttons; }
	bool upAll(u32 buttons) const { return (buttons & hold) == 0; }

	bool downTrigger(u32 buttons) const { return trig & buttons; }
	bool upTrigger(u32 buttons) const { return release & buttons; }

	Vector3f getAccel() const DECOMP_DONT_INLINE { return Vector3f(acc.x, acc.y, acc.z); }

	eCoreDevType getDevType() const { return static_cast<eCoreDevType>(dev_type); }

	bool isCore() const { return getDevType() == WPAD_DEV_CORE || isFreestyle(); }
	bool isFreestyle() const { return getDevType() == WPAD_DEV_FREESTYLE; }
...
public:
	...
	s32 getChannelID() const { return mChannelID; }

	Vector3f getAccel() const DECOMP_DONT_INLINE { return mCoreStatus[0].getAccel(); }

	eCoreDevType getDevType() const { return mCoreStatus[0].getDevType(); }

	bool isCore() const { return mCoreStatus[0].isCore(); }
	bool isFreestyle() const { return mCoreStatus[0].isFreestyle(); }

	s32 getReadLength() const { return mKPADReadLength; }

	bool isStable(u8 bits) const { return (mStableFlags & bits) == bits; }

	void setStableFrame(int frame) { mStableFrame = frame; }
	void setStableMag(f32 mag) { mStableMag = mag; }
	...
	CoreStatus* getCoreStatus() { return getCoreStatus(0); }
...
private:
	...
	void setFlags(u8 bit) { mFlags.setBit(bit); }
	void clrFlags(u8 bit) { mFlags.resetBit(bit); }
	bool tstFlags(u8 bit) { return mFlags.onBit(bit); }
